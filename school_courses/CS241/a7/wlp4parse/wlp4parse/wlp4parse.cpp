...
int main(){
    ...
    try {
        ...
        vector<string> temp;
        s = "BOF";
        pair<string, string> sp(s,s);
        lexemes.push_back(sp);
        ...
        pair<string, string> sp2(s,s);
        lexemes.push_back(sp2);
        while (temp.size()>0) {
            s = temp[temp.size()-1];
            sequence.push_back(s);
            temp.pop_back();
        ...
        while(sequence.size() > 0) {
            ...
            if (state < 0) {
                ...
                printingr.push_back(rule);
                printingl.push_back(correct);
        ...
        // accepted
        printingr.push_back(0);
        printingl.push_back(lexemes.size());
        for (i = 0; i < printingr.size(); i++) {
            if (i == 0) {
                for(int j = 0; j < printingl[0];j++) {
                    cout << lexemes[j].first << " " << lexemes[j].second << endl;
                }
            } else {
                for (int j = printingl[i-1]; j < printingl[i]; j++){
                    cout << lexemes[j].first << " " << lexemes[j].second << endl;
                }
            }
            for (int j = 0; j < rules[printingr[i]].size(); j++) {
                cout << rules[printingr[i]][j];
                if (j != rules[printingr[i]].size() - 1) {
                    cout << " ";
                } else {
                    cout << endl;
                }
